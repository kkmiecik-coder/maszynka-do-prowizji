...
- **`renderer/`** — plain HTML/CSS/JS (no framework). `index.html`/`main.ui.js` is the main 3-step flow (Źródła → Pliki → Wysyłka); `config.html`/`config.ui.js` is the separate config window. The renderer only calls `window.api.*`. Step 2 lists the generated files with an **"Otwórz"** button (`window.api.openFile` → `shell.openPath`, opens the `.xlsx` in Excel) — it no longer shows per-row emails or per-row send; sending happens in bulk in step 3. The app version is shown in the header (`window.api.getVersion`).
